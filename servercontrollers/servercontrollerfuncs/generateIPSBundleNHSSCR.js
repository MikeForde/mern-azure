...
function generateIPSBundleNHSSCR(ipsRecord, options = {}) {
            ...
            "contained": [
                {
                    "resourceType": "Organization",
                    "id": "manufacturer",
                    // Extracting manufacturer from immunization name from ending in brackets
                    "name": immunization.name.includes('(') && immunization.name.includes(')') ?
                        immunization.name.substring(immunization.name.lastIndexOf('(') + 1, immunization.name.lastIndexOf(')')) :
    ...
    const composition = {
        ...
        "resource": {
            ...
            "custodian": {
                "reference": `urn:uuid:${organizationUUID}`
            },
            "section": [
                {
                    ...
                    "entry": medicationStatements.map((medStatement) => ({
                        "reference": `urn:uuid:${medStatement.resource.id}`
                    }))
                },
                {
                    ...
                    "entry": allergyIntolerances.map((allergyIntolerance) => ({
                        "reference": `urn:uuid:${allergyIntolerance.resource.id}`
                    }))
                },
                {
                    "title": "Problems",
                ...
                },
                {
                    ...
                    "entry": immunizations.map((immunization) => ({
                        "reference": `urn:uuid:${immunization.resource.id}`
                    }))
                },
                {
                    "title": "Procedures",
                    ...
                    "entry": procedures.map((procedure) => ({
                        "reference": `urn:uuid:${procedure.resource.id}`
                    }))
                }
            ]
        }
                    ...
                    "name": [
                        {
                            ...
                            "prefix": ipsRecord.patient.gender === "female" ? ["Ms"] : ipsRecord.patient.gender === "male" ? ["Mr"] : undefined
                        }
                    ],
